...
//            $res = imagestring($image, $this->font, $x, $y, $text, $color);
//            if ($rotate){
...
//            $res = imagettftext($image, $this->size, $this->angle, $x, $y + $this->getHeight($text), $color, $this->font, $text);
//            if ($rotate){
//                $res = imagerotate($res, 90, 0);
//            }
//            return $res;
...
class Diagram {
    ...
    public function draw($format = "png") {
        ...
        $margins = [
            'top' => 0,
            'bottom' => $this->fontAxis->getHeight("9") * 1.5 + 33,
        ...
        */
        ...
        foreach ($this->data as $k => $v) {
            ...
            //
            $specialValue = ($this->fontAxis->getHeight($k)*1.5)*($i%2);
            //
            imageline($this->image,
                $x, $y, $x, $y + $this->fontAxis->getHeight("9") / 2 + $specialValue, $axisColor
            );
            //
            $this->fontAxis->draw($this->image, $x - $this->fontAxis->getWidth($k)/2, $y + $this->fontAxis->getHeight($k) / 2 + $specialValue, $k, $axisColor);
            //
            $hd = (($v - $min) / ($max - $min)) * $hy;
